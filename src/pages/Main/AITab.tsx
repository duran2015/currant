...
export function AITab() {
  const { currentTab, pushView, setSelectedCounselorId } = useAppStore();
  ...
  return (
    ...
    >
      <div className="pt-12 pb-2 px-6 bg-white sticky top-0 z-20 flex flex-col shadow-[0_1px_10px_rgba(0,0,0,0.02)]">
      ...
      {activeTab === "ai" && (
        <div className="bg-white/95 backdrop-blur-md px-4 py-3 border-b border-gray-100 flex items-center justify-between sticky top-[92px] z-20">
           ...
           <div className="flex items-center space-x-2">
             <button onClick={() => pushView("ai-chat-records")} className="text-gray-600 p-2 hover:bg-gray-50 rounded-full transition-colors flex items-center">
                <Clock size={20} />
             </button>
             <button className="text-gray-600 p-2 hover:bg-gray-50 rounded-full transition-colors flex items-center">
      ...
      {activeTab === "ai" && (
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#f8f9fa] via-[#f8f9fa] to-transparent pt-6 pb-6 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
          ...
          <div className="px-4 pb-2">
             <div className="flex items-center space-x-2 mb-2">
               <button className="w-10 h-10 rounded-full bg-white border border-gray-200 shadow-sm flex items-center justify-center text-gray-500 shrink-0 active:bg-gray-50 transition-colors">
                  <AudioLines size={18} />
               </button>
               <div className="flex-1 flex items-center bg-white rounded-full border border-gray-200 shadow-sm px-4 py-2 h-[42px] focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10 transition-all">
                 <input
                   value={input}
                   onChange={(e) => setInput(e.target.value)}
                   onKeyPress={(e) => { if(e.key === "Enter") { handleSend(); } }}
                   type="text"
                   className="flex-1 bg-transparent border-none outline-none text-[15px] text-gray-800 placeholder-gray-400 w-full"
                   placeholder="想对心愈说点什么..."
                 />
                 {input.trim() ? (
                   <button onClick={() => handleSend()} className="text-white bg-primary rounded-full w-7 h-7 ml-2 flex items-center justify-center shadow-md active:scale-95 transition-transform">
                     <Send size={14} className="translate-x-[-1px] translate-y-[1px]" />
                   </button>
                 ) : (
                   <button className="text-gray-400 ml-2 active:text-primary transition-colors">
                     <Mic size={18} />
                   </button>
                 )}
               </div>
